...
Runtime v0.1 complete and verified.
...
- A blocked task resumes to `ready` in v0.1. `blocked_from` is retained as provenance and as the future hook for restoring a prior state.
...
The YAML writer preserves readable data but does not round-trip comments. v0.1 avoids broad file rewriting and only writes the task DAG on explicit transitions.
